...
static jmethodID set_message_method_id, set_buttons_method_id, set_address_method_id, set_log_method_id, set_progress_method_id, set_servos_method_id;
...
void Java_pl_bezzalogowe_mavlink_MAVLinkClass_classInit(JNIEnv *env, jclass klass) {
    ...
    set_servos_method_id = (*env)->GetMethodID(env, klass, "setServos", "(SSSS)V");
}

void Java_pl_bezzalogowe_mavlink_MAVLinkClass_setGroundStationIP(JNIEnv *env, jclass klass, jstring ip) {
...
Java_pl_bezzalogowe_mavlink_MAVLinkClass_sendGlobalPosition(JNIEnv *env, jclass klass,
                                                       ...
                                                       double altitude, double relativeAltitude) {
    if (running) {
                ...
                if (mavlink_parse_char(MAVLINK_COMM_0, rcv_buf[i], &rcv_msg, &status) == 1) {
                    ...
                    switch (rcv_msg.msgid) {
                        ...
                        case 69: {
                            ...
                            //sprintf(feedback_ui, "axes: X: %d, Y: %d, Z: %d, R: %d\r\nbuttons: %s", manual_control.x, manual_control.y, manual_control.z, manual_control.r, buttons);
                            //set_ui_message_buttons(thiz, feedback_ui, true);

                            /** prints 1 for pressed, 0 for released */
