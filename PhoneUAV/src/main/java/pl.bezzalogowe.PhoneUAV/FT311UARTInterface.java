...
public class FT311UARTInterface extends Activity {
    ...
    public byte[] readBufferFB = new byte[4096];
    public char[] readBufferToChar = new char[4096];
    public int[] actualNumBytes = new int[1];

    public StringBuffer readSB = new StringBuffer();
...
/****************************************** USB handling ******************************************/
    ...
    public byte ReadData(int numBytes, byte[] buffer, int[] actualNumBytes) {
        ...
        for (int count = 0; count < numBytes; count++) {
            ...
            readIndex++;
			/*shouldn't read more than what is there in the buffer, so no need to check the overflow */
    ...
    public int ResumeAccessory() {
        ...
        if (accessory != null) {
            ...
            Toast.makeText(global_context, "Strings are matched! (UART mode)", Toast.LENGTH_SHORT).show();
